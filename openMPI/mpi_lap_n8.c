...
int main(int argc, char **argv)
{
	...
	while(dt > MAX_TEMP_ERROR && iteration <= max_iterations){
		switch(taskid){
			case MASTER:
				dt = _calc_avg(ROWS/4*3, ROWS, COLS/2, COLS);
				break;
			case NODE_1:
				dt = _calc_avg(ROWS/4*3, ROWS, 1, COLS/2-1);
				break;
			case NODE_2:
				dt = _calc_avg(ROWS/4*2, ROWS/4*3-1, COLS/2, COLS);
				break;
			case NODE_3:
				dt = _calc_avg(ROWS/4*2, ROWS/4*3-1, 1, COLS/2-1);
				break;
			case NODE_4:
				dt = _calc_avg(ROWS/4, ROWS/4*2-1, COLS/2, COLS);
				break;
			case NODE_5:
				dt = _calc_avg(ROWS/4, ROWS/4*2-1, 1, COLS/2-1);
				break;
			case NODE_6:
				dt = _calc_avg(1, ROWS/4-1, COLS/2, COLS);
				break;
			case NODE_7:
				dt = _calc_avg(1, ROWS/4-1, 1, COLS/2-1);
				break;
			default:
				break;
		}
